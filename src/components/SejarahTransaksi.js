...
const SejarahTransaksi = () => {
  ...
  const [itemSortConfig, setItemSortConfig] = useState({ key: "cost", direction: "desc" });
  ...
  useEffect(() => {
    ...
    const actualPath = isProduction ? "transactionDetail" : "transactionDetail_testing";
    ...
    queryCollection(
      ...
      .then((transactions) => {
        console.log(`Fetched ${transactions.length} transactions from ${environment} environment (${actualPath})`);
  ...
  useEffect(() => {
    ...
    console.log(`Querying penjualan transactions from collection: ${actualPath}`);
    ...
    queryCollection(
      ...
      .then((transactions) => {
        console.log(`Fetched ${transactions.length} penjualan transactions from ${environment} environment (${actualPath})`);
  ...
  }, [selectedChip, dateRange.start, dateRange.end, isProduction, itemSortConfig.key]);
  ...
  const handleItemSearch = (e) => {
    ...
    const filtered = itemsData.filter(item => 
      ...
      (item.subKategori && item.subKategori.toLowerCase().includes(searchTerm))
    );
  ...
  const openItemDialog = (item) => {
    ...
    setItemTransactions(item.transactions.sort(
      (a, b) => b.timestampInMillisEpoch.toDate() - a.timestampInMillisEpoch.toDate()
    ));
  ...
  return (
    <div className="sejarah-transaksi-container">
            ...
            <div className="items-table-container">
              <table className="items-table">
                <thead>
                  <tr>
                    <th 
                      className={`sortable ${itemSortConfig.key === 'itemName' ? 'sorted-' + itemSortConfig.direction : ''}`}
                      onClick={() => requestItemSort('itemName')}
                    >
                      Item Name
                    </th>
                    <th 
                      className={`sortable ${itemSortConfig.key === 'kategori' ? 'sorted-' + itemSortConfig.direction : ''}`}
                      onClick={() => requestItemSort('kategori')}
                    >
                      Category
                    </th>
                    <th 
                      className={`sortable ${itemSortConfig.key === 'subKategori' ? 'sorted-' + itemSortConfig.direction : ''}`}
                      onClick={() => requestItemSort('subKategori')}
                    >
                      Subcategory
                    </th>
                    <th 
                      className={`sortable center ${itemSortConfig.key === 'quantity' ? 'sorted-' + itemSortConfig.direction : ''}`}
                    ...
                    <th 
                      className={`sortable right ${itemSortConfig.key === 'revenue' ? 'sorted-' + itemSortConfig.direction : ''}`}
                      onClick={() => requestItemSort('revenue')}
                    >
                      Revenue
                    </th>
                    <th 
                      className={`sortable right ${itemSortConfig.key === 'stockWorth' ? 'sorted-' + itemSortConfig.direction : ''}`}
                      onClick={() => requestItemSort('stockWorth')}
                    >
                      Cost
                    </th>
                    <th 
                      className={`sortable right ${itemSortConfig.key === 'profitMargin' ? 'sorted-' + itemSortConfig.direction : ''}`}
                ...
                <tbody>
                  {filteredItemsData.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="right">{formatCurrency(Math.round(item.revenue))}</td>
                      <td className="right">{formatCurrency(Math.round(item.stockWorth))}</td>
                      <td className="right">{formatCurrency(Math.round(item.profitMargin))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {itemDialogOpen && selectedItem && (
        ...
        >
          <div className="dialog item-dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="item-details">
              <div className="item-detail-row">
                ...
                <span className="item-detail-value">{selectedItem.kategori || 'N/A'}</span>
              </div>
              <div className="item-detail-row">
                <span className="item-detail-label">Subcategory:</span>
                <span className="item-detail-value">{selectedItem.subKategori || 'N/A'}</span>
              </div>
              <div className="item-detail-row">
                <span className="item-detail-label">Total Revenue:</span>
                <span className="item-detail-value">{formatCurrency(Math.round(selectedItem.revenue))}</span>
              </div>
              <div className="item-detail-row">
                <span className="item-detail-label">Total Cost:</span>
                <span className="item-detail-value">{formatCurrency(Math.round(selectedItem.stockWorth))}</span>
              </div>
              <div className="item-detail-row">
                <span className="item-detail-label">Profit Margin:</span>
                <span className="item-detail-value">{formatCurrency(Math.round(selectedItem.profitMargin))}</span>
              </div>
            </div>
              ...
              <table className="transactions-table">
                ...
                <tbody>
                  {itemTransactions.map((tx, idx) => {
                    ...
                    const txQuantity = tx.originalQuantity !== undefined ? tx.originalQuantity : tx.quantity;
                    ...
                    return (
                      <tr key={idx}>
                        <td>{tx.timestampInMillisEpoch && tx.timestampInMillisEpoch.toDate ? 
                            tx.timestampInMillisEpoch.toDate().toLocaleDateString("id-ID") : 'N/A'}</td>
                        <td className="center">
                          {txQuantity} {tx.originalUnit || tx.unit}
                        </td>
                        <td className="right">{formatCurrency(Math.round(txRevenue))}</td>
                        <td className="right">{formatCurrency(Math.round(txCost))}</td>
                        <td className="right">{formatCurrency(Math.round(txProfit))}</td>
                      </tr>
                    );
                  })}
                </tbody>
